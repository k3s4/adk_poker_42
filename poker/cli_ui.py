...
class PokerUI:
    ...
    def _get_available_agents() -> List[str]:
        """
        agentsフォルダをスキャンして利用可能なエージェントを動的に取得
        
        Returns:
            利用可能なエージェント名のリスト
        """
        agents = []
        # プロジェクトルートのagentsディレクトリを取得
        current_file = Path(__file__)
        project_root = current_file.parent.parent  # poker/ -> project_root
        agents_dir = project_root / "agents"
        
        if not agents_dir.exists():
            print(f"警告: agentsディレクトリが見つかりません: {agents_dir}")
            return []
        
        # agentsディレクトリ内のサブディレクトリをスキャン
        for item in agents_dir.iterdir():
            if item.is_dir() and not item.name.startswith('_') and not item.name.startswith('.'):
                # agent.pyファイルが存在するかチェック
                agent_file = item / "agent.py"
                if agent_file.exists():
                    agents.append(item.name)
        
        return sorted(agents)  # ソートして一貫性を保つ
    ...
    def run_agent_only_mode(
        self, max_hands: int = 20, agents_config: str = "team1_agent:2,team2_agent:2", uuid_suffix: str = None
    ):
        """
        ...
        Args:
            ...
            uuid_suffix: 統一UUID（DB、ログ、結果ファイルで使用）
        """
        print("=== エージェント専用モード ===")
        ...
        # ゲームセットアップ
        self.game = PokerGame(uuid_suffix=uuid_suffix)
        ...
        try:
            ...
            # 結果をテキストファイルに保存
            self._save_agent_only_results(player_stats, agents_config, hand_count, uuid_suffix)

    def _save_agent_only_results(self, player_stats: Dict[str, Any], agents_config: str, hand_count: int, uuid_suffix: str = None):
        """エージェント専用モードの結果をテキストファイルに保存"""
        ...
        # タイムスタンプ付きのファイル名を生成（統一UUID付き）
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        if uuid_suffix is None:
            import uuid
            uuid_suffix = str(uuid.uuid4())[:4]  # 4桁のUUID
        filename = results_dir / f"agent_only_results_{timestamp}_{uuid_suffix}.txt"
        
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write("=" * 80 + "\n")
                f.write("エージェント専用モード ゲーム結果\n")
                f.write("=" * 80 + "\n")
                f.write(f"実行日時: {datetime.now().strftime('%Y年%m月%d日 %H:%M:%S')}\n")
                f.write(f"エージェント設定: {agents_config}\n")
                f.write(f"実行ハンド数: {hand_count}\n")
                f.write(f"初期チップ: {self.game.initial_chips if self.game else 'N/A'}\n")
                f.write("\n")
                ...
                # 最終チップ数でソート
                if self.game:
                    final_players = sorted(self.game.players, key=lambda p: p.chips, reverse=True)
                    for i, player in enumerate(final_players, 1):
                        stats = player_stats.get(player.name, {})
                        f.write(f"{i}. {player.name} ({stats.get('agent_type', 'Unknown')})\n")
                        f.write(f"   最終チップ: {player.chips}\n")
                        f.write(f"   勝利ハンド数: {stats.get('hands_won', 0)}\n")
                        f.write(f"   総獲得チップ: {stats.get('total_winnings', 0)}\n")
                        f.write(f"   参加ハンド数: {stats.get('hands_played', hand_count)}\n")
                        f.write(f"   勝率: {(stats.get('hands_won', 0) / max(stats.get('hands_played', 1), 1) * 100):.1f}%\n")
                        f.write("\n")
                ...
                for player_name, stats in player_stats.items():
                    ...
                    if self.game:
                        ...
                        if player:
                            agent_stats[agent_type]['total_chips'] += player.chips
                    agent_stats[agent_type]['total_wins'] += stats.get('hands_won', 0)
                    agent_stats[agent_type]['total_winnings'] += stats.get('total_winnings', 0)
                    agent_stats[agent_type]['total_hands'] += stats.get('hands_played', hand_count)
                
                for agent_type, stats in agent_stats.items():
                    f.write(f"{agent_type}:\n")
                    f.write(f"   プレイヤー数: {stats['players']}\n")
                    f.write(f"   平均チップ: {stats['total_chips'] / stats['players']:.0f}\n")
                    f.write(f"   総勝利数: {stats['total_wins']}\n")
                    f.write(f"   総獲得チップ: {stats['total_winnings']}\n")
                    f.write(f"   平均勝率: {(stats['total_wins'] / max(stats['total_hands'], 1) * 100):.1f}%\n")
                    f.write("\n")
                
                # ゲーム設定
                f.write("ゲーム設定:\n")
                f.write("-" * 50 + "\n")
                f.write(f"スモールブラインド: {self.game.small_blind if self.game else 'N/A'}\n")
                f.write(f"ビッグブラインド: {self.game.big_blind if self.game else 'N/A'}\n")
                f.write(f"プレイヤー数: {len(self.game.players) if self.game else 'N/A'}\n")
                f.write("\n")
    ...
    def _parse_agents_config(self, agents_config: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            プレイヤー設定のリスト
        """
        # agentsフォルダから動的に利用可能なエージェントを取得
        available_agents = self._get_available_agents()
        
        if not available_agents:
            print("警告: 利用可能なエージェントが見つかりませんでした。")
            # フォールバック: 既知のエージェントリスト
            available_agents = [
                "team1_agent",
                "team2_agent",
                "team3_agent",
