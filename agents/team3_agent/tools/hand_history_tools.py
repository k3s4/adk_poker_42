...
# ====== DB レイヤ ======
class PlayerStatsDB:
    def __init__(self, path: str = None, uuid_suffix: str = None):
        if path is None:
            path = self._generate_unique_db_path(uuid_suffix)
    ...
    def _generate_unique_db_path(self, uuid_suffix: str = None) -> str:
        ...
        # ファイル名を生成
        filename = f"poker_stats_{timestamp}_{uuid_suffix}.sqlite3"
        return os.path.join("./team3_agent/db", filename)
